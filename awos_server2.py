...
def get_latest():
    ...
    response_data = apply_offsets(latest_weather_data)
...
def get_history():
    """Get weather history with sensor offsets applied (display units: m/s, degC, hPa)"""
    return jsonify([apply_offsets(r) for r in weather_history])
...
def download_excel():
    ...
    try:
        ...
        for row_num, record in enumerate(filtered_data, 2):
            rec = apply_offsets(record)
            wind_speed_kt = round(float(rec.get('windSpeed', 0)) * 1.94384, 1)
            
            row_data = [
                record.get('utcDate', 'N/A'),
                record.get('utcTime', 'N/A'),
                record.get('localTime', 'N/A'),
                round(float(rec.get('temperature', 0)), 1),
                round(float(rec.get('humidity', 0)), 1),
                round(float(rec.get('pressure', 0)), 1),
                wind_speed_kt,
                round(float(rec.get('windDirection', 0)), 0),
                round(float(rec.get('dewPoint', 0)), 1),
    ...
    try:
        ...
        for row_num, record in enumerate(filtered_data, 2):
            ...
            row_data = [
                ...
                round(float(rec.get('humidity', 0)), 1),
                round(float(rec.get('pressure', 0)), 1),
                round(float(rec.get('windSpeed', 0)) * 1.94384, 1),
                round(float(rec.get('windDirection', 0)), 0),
                round(float(rec.get('dewPoint', 0)), 1),
                round(float(record.get('voltage', 0)), 2),
                round(float(record.get('current', 0)), 2),
